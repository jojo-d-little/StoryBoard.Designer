...
public sealed class SharedVariableInitialValueConsistencyRule : IValidationRule
{
    ...
    public ValidationRuleMetadata Metadata { get; } = new(
        RuleId: "PROJ-005",
        Title: "Shared Variable Initial Value Consistency",
    ...
    public IEnumerable<ValidationIssue> Evaluate(ValidationRuleContext context)
    {
        ...
        foreach (var entry in linkedVariablesBySharedId.Where(static pair => pair.Value.Count > 1))
        {
            ...
            var distinctInitialValues = participants
                .Select(static participant => participant.DefaultValue)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList();

            if (distinctInitialValues.Count <= 1)
            {
                continue;
            }
            ...
            var participantDetails = string.Join(
                ...
                participants.Select(participant =>
                    $"{participant.ScopePath}:{participant.VariableName}='{participant.DefaultValue}'"));

            yield return new ValidationIssue(
                Metadata.RuleId,
                Metadata.DefaultSeverity,
                "Project",
                $"Shared variable '{sharedLabel}' has mismatched participant initial values. Participants: {participantDetails}.");
        }
    }
