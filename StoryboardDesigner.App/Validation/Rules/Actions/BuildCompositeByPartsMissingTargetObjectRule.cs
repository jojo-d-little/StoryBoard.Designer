...
public sealed class BuildCompositeByPartsMissingTargetObjectRule : IValidationRule
{
    public ValidationRuleMetadata Metadata { get; } = new("ACT-011", "BuildCompositeByParts Missing Target Object", ValidationSeverity.Warning, "Actions");
    ...
    public IEnumerable<ValidationIssue> Evaluate(ValidationRuleContext ruleContext)
    {
        ...
        var targetObjectId = payload.CompositeTargetObjectId;
        if (!targetObjectId.HasValue || targetObjectId.Value == Guid.Empty)
        {
            yield break;
        }
        ...
        if (knownObjectIds.Contains(targetObjectId.Value))
        {
            yield break;
        }
        ...
        yield return new ValidationIssue(
            ...
            $"BuildCompositeByParts action '{action.Name}' references composite target object id '{targetObjectId.Value:N}' that does not exist in the project.");
    }
