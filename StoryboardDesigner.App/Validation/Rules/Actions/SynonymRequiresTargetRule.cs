...
public sealed class SynonymRequiresTargetRule : IValidationRule
{
    public ValidationRuleMetadata Metadata { get; } = new("ACT-001", "Synonym Requires Target", ValidationSeverity.Error, "Actions");
    ...
    public IEnumerable<ValidationIssue> Evaluate(ValidationRuleContext ruleContext)
    {
        ...
        {
            yield return new ValidationIssue(Metadata.RuleId, Metadata.DefaultSeverity, ActionRuleSupport.BuildActionIssuePath(context.Path, action), $"synonym action '{action.Name}' requires a target action.");
        }
    }
}
